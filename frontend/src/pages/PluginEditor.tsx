...
const CONSUMER_SUPPORT: Record<ComponentType, string[]> = {
  ...
  hooks: ["Claude Code", "Copilot"],
  agents: ["Claude Code", "Copilot"],
  "mcp-servers": ["Claude Code", "Copilot"],
  monitors: ["Claude Code"],
};
      ...
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-1.5 flex flex-wrap gap-1">
            {consumers.map((c) => (
              <span key={c} className={`rounded px-1.5 py-0.5 text-xs ${CONSUMER_COLORS[c] ?? "bg-slate-100 text-slate-600"}`}>{c}</span>
            ))}
          </div>
        </div>
