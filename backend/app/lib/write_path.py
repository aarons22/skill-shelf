...
def _claude_plugin_json(skill_slug: str) -> str:
    import json
    return json.dumps({
        "name": skill_slug,
...
def build_skill_files(
    ...
    return {
        f"{base}/.claude-plugin/plugin.json": _claude_plugin_json(skill_slug),
        f"{base}/.codex-plugin/plugin.json": _codex_plugin_json(
            skill_slug,
            display_name,
            description,
            version,
...
def remove_skill_files(skill_slug: str) -> dict[str, None]:
    ...
    return {
        ...
        f"{base}/.codex-plugin/plugin.json": None,
